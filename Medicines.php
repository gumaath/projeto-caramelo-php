...
if($_REQUEST && @$_POST['Pesquisar']) {
    $data = Functions::returnMedicineData(@$_POST['id_pet'], @$_POST['med'], @$_POST['date']);
}
...
<body>
    ...
    <div class="result container my-4" style="width: 80%;">        
        <div class="card">
            <div class="card-header"><?= $_data['name_med']?></div>
            <div class="card-body">
                <p class="card-text">Pet:  <?= $_data['pet'] ?></p>
                <p class="card-text">Orientação: <?= $_data['descricao'] ?></p>
                <span class="card-text">Data de aplicação:</span>
                <span class="text-muted d-inline"><?= Functions::formatDate($_data['date_med']) ?></span>
    ...
    <?php if(@$data == null && $_REQUEST && !@$_POST['Clear']) { ?>
        <div class="text-center container-fluid mt-5">
            <h4 class="mt-5">Não existem medicamentos aplicados com os parâmetros selecionados!</h4>
        </div>
    <?php } ?>
